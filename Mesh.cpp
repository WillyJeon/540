...
void Mesh::HelperBuffers(Vertex vertices[], unsigned int indices[], int numberIndices, int numberVertices, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
...
{
	HelperBuffers(vertices, indices, numberIndices, numberVertices, device);
}
...
Mesh::Mesh(const char* objFile, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	...
	obj.close();
	HelperBuffers(&verts[0], &indices[0], indexCounter, vertCounter, device);
